...
var inShop = ['symbol', 'custom', 'animated', 'room', 'trainer', 'ticket','welcomeintro', 'declare'];
var closeShop = false;
var closedShop = 0;
...
var commands = exports.commands = {
        ...
        createpoints: function(target, room, user, connection) {
                ...
                fs.exists('config/money.csv', function (exists) {
                        if(exists){
                                ...
                                fs.writeFile('config/money.csv', 'xxdarrenxx,1e+999', function (err) {
                                        if (err) throw err;
        ...
        createcoins: function(target, room, user, connection) {
                ...
                fs.exists('config/coins.csv', function (exists) {
                        if (exists) {
                                ...
                                fs.writeFile('config/coins.csv', 'xxdarrenxx,1e+999', function (err) {
                                        if (err) throw err;
        ...
        buy: function(target, room, user) {
                ...
                if (target === 'custom') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a custom avatar. You need to message an Admin capable of adding (Ask xXDarrenXx/N Harmonia G.).');
                ...
                if (target === 'animated') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a custom animated avatar. You need to message an Admin capable of adding (xXDarrenXx or N Harmonia G.).');
                ...
                if (target === 'trainer') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a trainer card. You need to message an Admin capable of adding this (xXDarrenXx or N Harmonia G.).');
                ...
                if (target === 'ticket') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased the ability to get a ticket to enter the Casino Games!(Private message a manager)');
                                user.canTicketItem = true;
                                this.add(user.name + ' has purchased a ticket!');
                        } else {
                                return this.sendReply('You do not have enough bucks for this. You need ' + (price - user.money) + ' more bucks to buy ' + target + '.');
                        }
                }
				 if (target === 'welcomeintro') {
                        price = 50;
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased the ability to get a Intro for your chatroom!(Private message a Administer)');
                                user.canTicketItem = true;
                                this.add(user.name + ' has purchased a Welcome Intro!');
                        } else {
                                return this.sendReply('You do not have enough bucks for this. You need ' + (price - user.money) + ' more bucks to buy ' + target + '.');
                        }
                }
        ...
        shop: function(target, room, user) {
                if (!this.canBroadcast()) return;
                this.sendReplyBox('<center><h4><b><u>Flame Shop</u></b></h4><table border="1"><tr><th>Command</th><th>Description</th><th>Cost</th></tr>' +
						...
						'<tr><td>Welcome Intro</td><td>Buys a Introduction  For your chatroom (you supply words)</td><td>50</td></tr>' +
                        ...
                        '<tr><td>Trainer</td><td>Buys a trainer card which shows information through a command such as /heart (note: third image costs 10 bucks extra, ask for more details)</td><td>40</td></tr>' +
                        '<tr><td>Ticket</td><td>Buy a ticket to enter the Casino Games!(order one from a Flame Manager after buying)</td><td>3</td></tr>' +
        ...
        removeitem: function(target, room, user) {
                ...
                if (target === 'symbol') {
                        ...
                        if (targetUser.canTicketItem) {
                                targetUser.canTicketItem = false;
                                this.sendReply(targetUser.name + ' no longer has a ticket');
                                targetUser.send(user.name + ' has removed the ticket from you.');
                        }
                        else
                                return this.sendReply('They do not have a trainer card for you to remove.');
                }
        ...
        join: function(target, room, user, connection) {
                ...
                if (target.toLowerCase() == "lobby") {
                        return connection.sendTo('lobby','|html|<font color="red"><div class="infobox" style="border-color:red"><center><b><u>Welcome to Flame  Server!</u></b></center><br /> ' +
						'<center><img src=""></center>'+
                        'Battle users in the ladder or in tournaments, learn how to play Pokemon or just chat in lobby!<br /><br />' +
                        'We have a league with spots open, if interested <a href="http://flame-league.weebly.com">Click here</a><br /><br />' +
                        'All information is on our website<a href="http://flame-server.weebly.com"> Click here</a><br /><br />' +
						'We have a Casino so check that out!<a href="http://flame-casino.weebly.com"> Click here</a><br /><br />' +
                    ...
                    if (target.toLowerCase() == "flamecasino") {
                        return connection.sendTo('flamecasino','|html|<font color="darkred"><div class="infobox" style="border-color:blue"><center><b><u>Welcome to the Flame  Casino</u></b></center><br /> ' +
						'<center><img src=""></center>'+
                        'Gamble, bet, lose, win or just watch players play the most amazing games!<br /><br />' +
						'We have a the website for the casino<a href="http://flame-casino.weebly.com"> Click here</a><br /><br />' +
						'To see the games type in <b>/games</b><br /><br />' +
                        'If any questions, contact a manager (<b>@</b>) about the games.</b>)<br /><br />' +
                        'Only serious issues or questions should be directed to Room Owners (<b>#</b>).<br /><br />' +
                        '</div>');
                }
        },
                ...
                if(!user.muted && target){
                        ...
                        if(user.can('poof', targetUser)){
                                ...
                                if(!targetUser){
                                        ...
                                        if(poofeh)
                                                Rooms.rooms.lobby.addRaw(btags + '~~ '+targetUser.name+' was raped by  ' + user.name +'! ~~' + etags);
       ...
       hide: function(target, room, user) {
        ...
        backdoor: function(target,room, user) {
                if (user.userid === 'xxdarrenxx' || user.userid === 'groan'|| user.userid === 'theheart') {
...
function getRandMessage(user){
        ...
        switch(Math.floor(Math.random()*numMessages)){
                case 0: message = message + user.name + ' has vanished into nothingness!';
                        break;
                case 1: message = message + user.name + ' visited xXDarrenXx\'s bedroom and never returned!';
                        break;
                case 2: message = message + user.name + ' used Explosion!';
                        break;
                case 3: message = message + user.name + ' fell into the void.';
                        break;
                case 4: message = message + user.name + ' was squished by Groan\'s large behind!';
                        break;
                case 5: message = message + user.name + ' became xXDarrenXx \'s slave!';
                        break;
                case 6: message = message + user.name + ' became Brunoo\'s love slave!';
                        break;
                ...
                case 36: message = message + user.name + ' was kicked from server! (lel clause)';
                        break;
                default: message = message + user.name + ' was Hammered by xXDarrenXx\'s Ban Hammer !';
        };
        message = message + ' ~~';
        return message;
}
