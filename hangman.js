...
var cmds = {   
        hangmanhelp: function(target, room, user) {
                ...
                this.sendReplyBox('<font size = 2>A brief introduction to </font><font size = 3>Hangman:</font><br />' +
                                                ...
                                                'Have fun, and feel free to PM me if you find any bugs! - N Harmonia');
        },
        ...
        hangman: function(target, room, user) {
                ...
                if(room.id === 'staff') {
                                return this.sendReply('|html|This is not time to play hangman in staff!!!>:I');
                }
                ...
                if(hangman[room.id].hangman === false) {
                        ...
                        if(targets[0].length > 10) {
                                return this.sendReply('Seeing as there are only 10 given guesses, don\'t make the word too long.')
                        }
                        ...
                        return this.add('|html|<div class = "infobox"><div class = "broadcast-green"><center><font size = 2><b>' + user.name + '</b> started a game of hangman!This game gives 3 points! The word has ' + targets[0].length + ' letters.<br>' + hangman[room.id].spaces.join(" ") + '<br>The category: ' + hangman[room.id].hangmantopic[0] + '</font></center></div></div>');
                }
        },
 
        viewhangman: function(target, room, user) {
                if(room.id === 'staff') {
                                return this.sendReply('|html|What did I say about no hangman in Staff?!?!');
                }
                if (!this.canBroadcast()) return;
                if(hangman[room.id].hangman === false) {
         ...
         category: function(target, room, user) {
                if(room.id === 'Staff') {
                                return this.sendReply('|html|Please play this in another room; it\'s too spammy for Staff');
                }
                if(hangman === false) {
                        return this.sendReply('There is no game of hangman going on right now.');
                }
        ...
        word: function(target, room, user) {
                if(room.id === 'staff') {
                                return this.sendReply('|html|There is no hangman in Staff.');
                }
        ...
        guess: function(target, room, user) {
                if(room.id === 'staff') {
                                return this.sendReply('|html|There isno hangman in Staff');
                }
                if(hangman[room.id].hangman === false) {
                ...
                if(hangman[room.id].correctletters.length === hangman[room.id].guessword[0].length) {
                        this.add('|html|Congratulations!You have won 3 points(it this room is official). <b>' + user.name + '</b> has guessed the word, which was: \'' + hangman[room.id].guessword[0] + '\'.');
                        hangman.reset(room.id)
                }
        },
        ...
        guessword: function(target, room, user) {
                ...
                if(targetword === hangman[room.id].guessword[0]) {
                        this.add('|html|Congratulations!You have won 3 points(if this chatroom is Official) <b>' + user.name + '</b> has guessed the word, which was: \'' + hangman[room.id].guessword[0] + '\'.');
                        hangman.reset(room.id)
                }
                else {
